 ...
 * Uso: npx ts-node tests/extract-financial-data.ts
 */
...
async function login(): Promise<string> {
  ...
  try {
    ...
    const token = loginData.data?.data?.accessToken || loginData.data?.accessToken || loginData.accessToken
...
async function getFinancialSnapshot(token: string, month: number, year: number) {
  ...
  try {
    ...
    return {
      faturamento: parseFloat(snapshot.receita || snapshot.totalRevenue || snapshot.faturamento || '0'),
      despesasVariaveis: parseFloat(snapshot.despesasVariaveis || snapshot.expenses || '0'),
      despesasFixas: parseFloat(snapshot.fixedExpensesTotal || snapshot.expenseFixed || '0'),
      lucroBruto: parseFloat(snapshot.grossProfit || snapshot.lucrobruto || '0'),
      lucroLiquido: parseFloat(snapshot.netProfit || snapshot.lucroliquido || '0'),
    }
  } catch (error) {
    console.error(`❌ Erro ao obter snapshot de ${month}/${year}:`, error)
    return null
  }
}
...
async function main() {
  ...
  try {
    ...
    // 4. Gerar relatório
    const monthNames = new Intl.DateTimeFormat('pt-BR', { month: 'long' }).format(now)
    const lastMonthName = new Intl.DateTimeFormat('pt-BR', { month: 'long' }).format(lastMonthDate)
    ...
    console.log(`🏢 Período: ${monthNames.toUpperCase()}/${currentYear}\n`)
    ...
    if (currentData) {
      console.log(`Faturamento:          ${formatCurrency(currentData.faturamento)}`)
      console.log(`Despesas Variáveis:   ${formatCurrency(currentData.despesasVariaveis)}`)
      console.log(`Despesas Fixas:       ${formatCurrency(currentData.despesasFixas)}`)
      console.log(`Total de Despesas:    ${formatCurrency(currentData.despesasVariaveis + currentData.despesasFixas)}`)
      console.log('-'.repeat(60))
      console.log(`💵 Lucro Bruto:        ${formatCurrency(currentData.lucroBruto)}`)
      console.log(`💵 Lucro Líquido:      ${formatCurrency(currentData.lucroLiquido)}`)

      if (currentData.faturamento > 0) {
        const margemBruta = ((currentData.lucroBruto / currentData.faturamento) * 100).toFixed(2)
        const margemLiquida = ((currentData.lucroLiquido / currentData.faturamento) * 100).toFixed(2)
    ...
    if (lastData) {
      ...
      console.log(`Despesas Variáveis:   ${formatCurrency(lastData.despesasVariaveis)}`)
      console.log(`Despesas Fixas:       ${formatCurrency(lastData.despesasFixas)}`)
      console.log(`Total de Despesas:    ${formatCurrency(lastData.despesasVariaveis + lastData.despesasFixas)}`)
      console.log('-'.repeat(60))
      console.log(`💵 Lucro Bruto:        ${formatCurrency(lastData.lucroBruto)}`)
      console.log(`💵 Lucro Líquido:      ${formatCurrency(lastData.lucroLiquido)}`)

      if (lastData.faturamento > 0) {
        const margemBruta = ((lastData.lucroBruto / lastData.faturamento) * 100).toFixed(2)
        const margemLiquida = ((lastData.lucroLiquido / lastData.faturamento) * 100).toFixed(2)
        console.log(`📊 Margem Bruta:       ${margemBruta}%`)
        console.log(`📊 Margem Líquida:     ${margemLiquida}%`)
      }
    } else {
    ...
    // ========== COMPARATIVO ==========
    if (currentData && lastData && currentData.faturamento > 0 && lastData.faturamento > 0) {
      console.log('📈 COMPARATIVO MÊS A MÊS')
      console.log('-'.repeat(60))

      const crescimentoFaturamento = (
        ((currentData.faturamento - lastData.faturamento) / lastData.faturamento) *
        100
      ).toFixed(2)
      const crescimentoLucro = (
        ((currentData.lucroLiquido - lastData.lucroLiquido) / Math.abs(lastData.lucroLiquido || 1)) *
        100
      ).toFixed(2)
      ...
      console.log(`${sinal1} Crescimento de Faturamento: ${crescimentoFaturamento}%`)
