...
export async function POST() {
  ...
  if (!ALLOWED_ROLES.includes(role)) {
    return NextResponse.json({ error: "Sem permissão para abrir a loja" }, { status: 403 })
  }

  try {
    const res = await fetch(`${BACKEND_URL}/api/store/open`, {
      method: "POST",
      headers: {
    ...
    console.error("[BFF /api/store/open] Error:", err)
